 ...
 *
 * Each id is a stand-in from the library until the undead's own recordings
 * land; the ids are preloaded with the village (`briarHollow` in
 * `sfxGroups.ts`), so none of them depends on which other floor-3 groups
 * happen to be loaded.
 */
