...
import { BUILDING_TILE_SCALE, type BuildingSpec } from './spec';
 ...
 *
 * Declared read-heavy, because that is what a plane is for: every texture and
 * lighting pass over it reads the whole buffer back and writes it again, half a
 * dozen times before the plane is drawn once. A GPU-backed surface pays a full
 * synchronisation for each of those reads — measured in a browser, that was most
 * of the cost of painting a building.
 */
export function makePlane(quad: Quad, widthOverride?: number, heightOverride?: number): Plane {
  ...
  const canvas = allocReadableCanvas(width, height);
  return { canvas, ctx: surfaceContext(canvas), width, height, quad };
}
...
export function quadPath(ctx: Ctx, quad: Quad): void {
  ctx.beginPath();
 ...
 * solved per pixel.
 */
export function drawPlane(target: Ctx, plane: Plane): void {
