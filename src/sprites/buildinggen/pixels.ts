...
import { surfaceContext, type CanvasSurface } from '../../core/canvasSurface';
...
export function readPixels(ctx: Ctx, width: number, height: number): PixelBuffer {
  const image = ctx.getImageData(0, 0, width, height);
  return { data: image.data, width, height };
}

export function writePixels(ctx: Ctx, buffer: PixelBuffer): void {
  const image = ctx.createImageData(buffer.width, buffer.height);
  image.data.set(buffer.data);
  ctx.putImageData(image, 0, 0);
}

export function readCanvas(canvas: CanvasSurface): PixelBuffer {
  return readPixels(surfaceContext(canvas), canvas.width, canvas.height);
}
