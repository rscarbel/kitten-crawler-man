...
import { paintEdgeAo, paintRecessAo, paintEdgeLight, paintWindowGlow } from '../lighting';
import { groundHeightToFrameY } from '../projection';
import type { Projection } from '../projection';
import { getRamp, hueShift, rgb, rgba, sampleRamp, type Ramp, type RGB } from '../ramps';
import type { PropKind, PropSpec, ReliefDevice } from '../spec';
import { jointWander } from '../texture';
import { ELEMENT_HUE_JITTER, ELEMENT_TONE_JITTER, elementValue } from '../materials/kit';
 ...
 */
function paintCastShadow(draw: PropDraw, trace: (ctx: Ctx) => void): void {
  const { ctx, box } = draw;
...
function paintHerbBundle(draw: PropDraw): void {
  ...
  const traceMass = (target: Ctx): void => {
    target.beginPath();
...
function paintCharmString(draw: PropDraw): void {
  ...
  const traceCord = (target: Ctx): void => {
    target.save();
...
function traceSuitPath(ctx: Ctx, suit: number, x: number, y: number, size: number): void {
  const half = size / 2;
  ctx.beginPath();
  if (suit === 0) {
...
function traceArchPath(ctx: Ctx, box: PropBox, springY: number): void {
  ctx.beginPath();
...
function paintLeanTo(draw: PropDraw): void {
  ...
  const traceRoof = (target: Ctx): void => {
    target.beginPath();
 ...
 */
type DeviceTracer = (ctx: Ctx, aspect: number) => void;
...
function traceCircle(ctx: Ctx, x: number, y: number, radius: number, anticlockwise = false): void {
  ctx.moveTo(x + radius, y);
  ctx.arc(x, y, Math.max(Number.MIN_VALUE, radius), 0, TWO_PI, anticlockwise);
  ctx.closePath();
}

function traceRect(ctx: Ctx, x: number, y: number, width: number, height: number): void {
  ctx.moveTo(x, y);
...
function traceCrossedWeapons(ctx: Ctx): void {
  ctx.beginPath();
...
function traceSword(ctx: Ctx): void {
  const TIP_Y = -1.14;
...
function traceSunDisc(ctx: Ctx): void {
  ctx.beginPath();
...
function traceMortarAndPestle(ctx: Ctx): void {
...
function traceHorn(ctx: Ctx): void {
  const MOUTH_X = -0.72;
...
function traceWheatSheaf(ctx: Ctx): void {
...
function traceTankard(ctx: Ctx): void {
  const TOP = -0.5;
  const BOTTOM = 0.72;
...
function traceWheel(ctx: Ctx, aspect: number): void {
...
const LETTER_STROKES: Readonly<Record<string, ReadonlyArray<LetterStroke> | undefined>> = {
  ' ': [],
  A: [
    [0, 1, 0.5, 0],
    [0.5, 0, 1, 1],
