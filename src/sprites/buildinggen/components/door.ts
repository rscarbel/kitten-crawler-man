...
import { paintEdgeAo, paintEdgeLight, paintRecessAo, paintStructuralLine } from '../lighting';
import {
...
import type { Plane, Projection } from '../projection';
import { tileColumnToPlaneX } from '../projection';
import {
  LIGHT_DIR_X,
  LIGHT_DIR_Y,
  getRamp,
...
import type { BuildingSpec, DoorArch, DoorSpec, DoorStyle } from '../spec';
import { jointWander, planeNoise } from '../texture';
 ...
 */
function openingPath(ctx: Ctx, rect: Rect, arch: DoorArch): void {
 ...
 */
function paintHeadInterior(ctx: Ctx, opening: Rect, arch: DoorArch): void {
...
function paintReveal(ctx: Ctx, opening: Rect, ramp: Ramp): void {
  ctx.save();
 ...
 */
function paintPlankLeaf(ctx: Ctx, noise: NoiseField, seed: number, leaf: Rect, ramp: Ramp): void {
...
function paintMeetingStile(ctx: Ctx, inner: Rect, leafWidth: number, ramp: Ramp): void {
...
function paintGrandFrame(ctx: Ctx, leaf: Rect, inner: Rect, ramp: Ramp): void {
  ctx.fillStyle = rgb(sampleRamp(ramp, Math.min(1, 0.5 + PANEL_TONE_LIFT)));
...
function paintRaisedPanels(ctx: Ctx, leafRect: Rect, ramp: Ramp): void {
 ...
 */
function paintBeadCurtain(ctx: Ctx, noise: NoiseField, seed: number, leaf: Rect, ramp: Ramp): void {
  ctx.save();
  ctx.beginPath();
  ctx.rect(leaf.x, leaf.y, leaf.width, leaf.height);
  ctx.clip();
...
function paintArch(ctx: Ctx, seed: number, opening: Rect, arch: DoorArch): void {
  switch (arch) {
    case 'none':
      return;
    case 'flat_lintel':
...
function paintLintel(ctx: Ctx, opening: Rect): void {
 ...
 * points are not.
 */
function paintVoussoirRing(ctx: Ctx, seed: number, ring: VoussoirRing): void {
 ...
 */
function paintHardware(ctx: Ctx, seed: number, leaf: Rect, door: DoorSpec): void {
 ...
 */
function paintSunkenWell(ctx: Ctx, opening: Rect, baseY: number, sunkenPx: number): void {
