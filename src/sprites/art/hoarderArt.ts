 ...
 * choreography lives in `hoarderFigure.ts`.
 */

import { type Pt, clamp01, deg, hump, lerp, mix } from './carlArt';
...
export function torsoOutlineCurvature(pose: HoarderPose, view: HoarderView): OutlineCurvature {
  ...
  for (const sign of [1, -1]) {
    ...
    if (segments.length > 0) points.push(segments[segments.length - 1].to);
