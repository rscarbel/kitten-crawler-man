 ...
 *
 * The palette is copied from `tusklingArt.ts` rather than imported: that
 * module exports no colours and no primitives, and `tusklingGore.ts` set
 * the precedent of re-deriving its own tones.
 */

import { clamp01, easeInOut, lerp, mix, type Pt } from './carlArt';
import { hash1, rgba } from './ratArt';
import { BOS_BODY_RADIUS_TILES } from '../ballOfSwineSheet';
...
export function drawBallSlam(ctx: Ctx, progress: number): void {
  ...
  // Spray thrown forward off the impact face. It travels on `progress` and not
  // on `crush`, because muck that has left the body does not come back into it:
  // hung off the compression it flew out and was sucked home again, and since
  // the compression is the row's only other term — a hump peaking at the
  // middle — the four declared frames painted two pictures, the second half of
  // the slam retracing the first.
  for (let i = 0; i < SPRAY_FLECKS; i++) {
    const spread = (hash1(i * 2.3 + 0.7) * 2 - 1) * 0.9;
    const flung = clamp01(progress);
    const distance = lerp(BALL_RADIUS * 0.95, SPRAY_REACH, flung * hash1(i * 2.3 + 1.1));
    ...
    fillEllipse(
      ...
      r,
      r * 0.8,
      tinted(i % 3 === 0 ? BLOOD : MUCK.mid, (0.55 + crush * 0.35) * (1 - flung * SPRAY_THINNING)),
    );
  }
}

/** Flecks of muck and blood thrown off the face of the ball that hit the wall. */
const SPRAY_FLECKS = 20;
/**
 * How much of its opacity a fleck has lost by the end of the row.
 *
 * Spray spreads as it flies, so the further out it gets the less of it there is
 * in any one place. Without the fade the flecks read as solid pellets shot out
 * of a barrel rather than as a thrown sheet coming apart.
 */
const SPRAY_THINNING = 0.7;
