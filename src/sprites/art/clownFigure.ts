/**
 * Grimaldi's clowns as painted figures: the choreography, the cell geometry and
 * the `FigureDef` the runtime cache, the parity run and the review harness all
 * draw through.
 *
 * This module is choreography and nothing else — the gait construction, one
 * pose function per row, the props each clown carries, the row tables and the
 * placement of a pose or a severed piece inside its cell. Anatomy, palette and
 * every stroke of paint live in `clownArt.ts` and `clownGore.ts`.
 *
 ...
 *
 * The art invariants live in `scripts/gates-clowns.ts`, which the review
 * harness runs: `npm run render:clowns`.
 */
...
export const FAT_STYLE: ClownStyle = {
  palette: FAT_PALETTE,
...
export const SLAM_WINDUP_END = 0.35;
...
export const STILT_STYLE: ClownStyle = {
...
export function terrorStyle(enraged: boolean): ClownStyle {
  return {
...
function makeMalletPainter(palette: ClownPalette): PropPainter {
  return (ctx: CanvasRenderingContext2D, hand: Point, wristAngle: number): void => {
    ctx.save();
...
export const EVIL_STYLE: ClownStyle = {
...
/**
 * How far the arms trail the stride that swings them, in cycle turns.
 *
 * These are the longest limbs on the figure — the hands hang past the knees —
 * and nothing about this clown suggests he is holding them anywhere: the
 * shoulder goes and the hand arrives afterwards, which is the drag that makes a
 * long arm read as slack rather than as a rod bolted to the ribs.
 *
 * Head-on it is also the only term that tells the two halves of the stride
 * apart. Each foot is planted for well over half its own cycle, so both are
 * home at rest on the frames that open each half; the bob and the lean run at
 * twice the stride rate and the pelvis sway is zero at both. With the arms on
 * the stride's own sine as well, the frame at mid-row was the opening frame
 * painted a second time.
 */
const FRONTAL_ARM_DRAG_TURNS = 0.25;
...
function frontalWalkPose(style: ClownStyle, phase: number, config: GaitConfig): ClownPose {
  const rest = restingPose(style);
  const swingAngle = phase * FULL_CYCLE;
  const armPhase = Math.sin(swingAngle - FRONTAL_ARM_DRAG_TURNS * FULL_CYCLE);
...
/**
 * How much of the chest he starts the laugh with is left at the end of it.
 *
 * A laugh is spent air. The first convulsion has a full chest behind it and
 * every one after it has less, so the swell and the gape have to shrink across
 * the row or he is not laughing, he is idling with his mouth open.
 *
 * It is also the only thing that tells the two heaves apart. Every other term
 * of the row is either flat across the held plateau or a term of that one heave
 * cycle, and two heaves put the third and eighth frames on the same instant of
 * it — the same instant, at that, being a zero crossing, so decaying the heave
 * itself would have scaled nothing. What decays has to be what is still
 * standing at the crossing: the chest and the gape.
 */
const LAUGH_BREATH_LEFT = 0.55;
/** How far the chest swells while he is laughing, before the air runs out. */
const LAUGH_CHEST_SWELL = 0.06;
/** How much more the chest jumps on each heave. */
const LAUGH_CHEST_HEAVE = 0.04;
/** How far the jaw hangs open through the laugh, before the air runs out. */
const LAUGH_GAPE = 0.7;
/** How much wider the jaw goes on each heave. */
const LAUGH_GAPE_HEAVE = 0.3;
...
function evilLaughPose(progress: number): ClownPose {
  ...
  const breathLeft = lerp(1, LAUGH_BREATH_LEFT, clamp01(progress));
  ...
  return {
    ...
    torsoSquash: 1 + (LAUGH_CHEST_SWELL * breathLeft + LAUGH_CHEST_HEAVE * heave) * held,
    nearHand: {
    ...
    headLead: -0.06 * held,
    mouthOpen: held * (LAUGH_GAPE * breathLeft + LAUGH_GAPE_HEAVE * heave),
...
/**
 * How far above his own feet the juggling hands hold, in tiles.
 *
 * Exported because `EvilClown` releases a thrown vial from exactly there, and a
 * bottle that leaves from anywhere else pops out of his shoes or out of empty
 * air above his hands. Derived rather than copied, so the two cannot drift.
 */
export const EVIL_CLOWN_JUGGLE_HAND_HEIGHT_TILES =
  shoulderHeight(EVIL_PROPORTIONS) - EVIL_ARM_LENGTH * JUGGLE_HAND_DROP;
...
function paintVial(ctx: CanvasRenderingContext2D): void {
...
export interface RowSpec {
  ...
  readonly pieces?: ReadonlyArray<(ctx: CanvasRenderingContext2D, style: ClownStyle) => void>;
  /**
...
export const FAT_SLAM_FRAMES = 10;
export const STILT_WINDUP_FRAMES = 6;
export const STILT_LUNGE_FRAMES = 8;
export const TERROR_WINDUP_FRAMES = 8;
export const TERROR_SWING_FRAMES = 8;
export const EVIL_SWIPE_FRAMES = 8;
export const EVIL_LAUGH_FRAMES = 10;
export const EVIL_JUGGLE_FRAMES = 8;
...
/**
 * The severed piece a gore state paints, or null when the state is a pose row.
 *
 * A generator's "gore row" is not one animation state: each piece is its own
 * one-frame state, so the row disappears into the state table and the piece
 * list is what is left of it.
 */
function gorePieceOf(
  spec: SheetSpec,
  state: string,
): { paint: (ctx: CanvasRenderingContext2D, style: ClownStyle) => void; style: ClownStyle } | null {
  for (const row of spec.rows) {
    const states = row.pieceStates;
    const pieces = row.pieces;
    if (states === undefined || pieces === undefined) continue;
    const index = states.indexOf(state);
    if (index < 0) continue;
    // The two lists are written side by side, so a length mismatch is a piece
    // that was named and never painted — silence there is a missing body part.
    if (index >= pieces.length) {
      throw new Error(`${spec.id} declares gore state "${state}" with no painter`);
    }
    return { paint: pieces[index], style: row.style };
  }
  return null;
}

function poseRowOf(spec: SheetSpec, state: string): RowSpec | undefined {
  return spec.rows.find((row) => row.name === state && row.pose !== null);
}

/**
 * Paints one cell of a clown, in the cell's own pixels.
 *
 ...
 * cell is the spin the gore field applies about its measured ink centre.
 */
function paintClownFrame(
  spec: SheetSpec,
  ctx: CanvasRenderingContext2D,
  state: string,
  frame: number,
): void {
  const gore = gorePieceOf(spec, state);
  if (gore !== null) {
    ctx.save();
    ctx.translate(spec.frameWidth / 2, spec.frameHeight / 2);
    ctx.scale(TILE_SCALE, TILE_SCALE);
    gore.paint(ctx, gore.style);
    ctx.restore();
    return;
  }
  const row = poseRowOf(spec, state);
  if (row === undefined) throw new Error(`${spec.id} paints no state "${state}"`);
  const pose = row.pose;
  if (pose === null) throw new Error(`${spec.id} row "${state}" has no pose function`);
  ctx.save();
  drawClown(
    ctx,
    spec.frameWidth / 2,
    ...
    row.style,
    pose(frame),
    row.propFor?.(frame) ?? row.prop,
    row.view,
  );
  ctx.restore();
}

/** Every state a sheet spec declares: pose rows by name, gore pieces by piece. */
function stateFramesOf(spec: SheetSpec): Record<string, number> {
  const frames: Record<string, number> = {};
  for (const row of spec.rows) {
    if (row.pieceStates === undefined) {
      frames[row.name] = row.frameCount;
      continue;
    }
    for (const state of row.pieceStates) frames[state] = 1;
  }
  return frames;
}

/**
 * The cell geometry each clown's bake measured, turned into a `FigureDef`.
 *
 * `tileX` and `tileY` are the same two numbers the old manifest carried: the
 * tile is centred in the cell and hangs off the ground line the poses stand on.
 */
function figureOf(spec: SheetSpec): FigureDef {
  return {
    id: spec.id,
    ...
    states: figureStates(stateFramesOf(spec)),
    paintFrame: (ctx, state, frame) => {
      paintClownFrame(spec, ctx, state, frame);
    },
  };
}

function specNamed(id: FigureId): SheetSpec {
  const spec = SHEETS.find((candidate) => candidate.id === id);
  if (spec === undefined) throw new Error(`no clown sheet spec named "${id}"`);
  return spec;
}

export const FAT_CLOWN_SPEC = specNamed('fat_clown');
export const STILT_CLOWN_SPEC = specNamed('stilt_clown');
export const TERROR_CLOWN_SPEC = specNamed('terror_clown');
export const EVIL_CLOWN_SPEC = specNamed('evil_clown');

export const FAT_CLOWN_FIGURE = figureOf(FAT_CLOWN_SPEC);
export const STILT_CLOWN_FIGURE = figureOf(STILT_CLOWN_SPEC);
export const TERROR_CLOWN_FIGURE = figureOf(TERROR_CLOWN_SPEC);
export const EVIL_CLOWN_FIGURE = figureOf(EVIL_CLOWN_SPEC);

/** Every clown figure, in the order the review harness lays them out. */
export const CLOWN_FIGURES: readonly FigureDef[] = [
  FAT_CLOWN_FIGURE,
  STILT_CLOWN_FIGURE,
  TERROR_CLOWN_FIGURE,
  EVIL_CLOWN_FIGURE,
...
export const CLOWN_SPECS: readonly SheetSpec[] = [
  FAT_CLOWN_SPEC,
  STILT_CLOWN_SPEC,
  TERROR_CLOWN_SPEC,
  EVIL_CLOWN_SPEC,
...
/** The spec a figure was built from, for gates that need the row table. */
export function clownSpecOf(def: FigureDef): SheetSpec {
  return specNamed(def.id);
}
