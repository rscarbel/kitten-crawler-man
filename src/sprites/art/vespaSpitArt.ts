 ...
 */
/**
 * Where in its own cycle a glob frame sits. Divided by the frame count rather
 * than by one less: the row loops, so the frame after the last has to land on
 * phase 1 — which is phase 0 — and not on it one frame early.
 * `scripts/gates-vespa-spit.ts` asserts exactly that, because the pixels
 * cannot: the glob's pulse decorrelates within a frame of any phase offset, and
 * a cycle running long measures a *smaller* wrap than the shipped one.
 */
export function projectilePhase(frame: number, frameCount: number): number {
...
export function drawVespaAcidProjectile(
  ...
  const phase = projectilePhase(frame, frameCount);
