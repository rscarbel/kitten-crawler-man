/**
 * Grotesque Spider art — the most terrifying creature in the game.
 *
...
/**
 * One of the eight legs: where it attaches to the body, where its tip rests,
 * and the clock it steps on. Offsets are in tiles from the body centre.
 *
 ...
 * see; only the rig itself knows where the eighth foot should be.
 */
...
/** The eight legs, front → back down the left side and then the right. */
export const SPIDER_LEGS: readonly SpiderLegDesc[] = [
...
/**
 * Where a leg's foot lands for a pose, in the target's coordinate space.
 *
 * Exported alongside {@link SPIDER_LEGS} so a gate can paint a pose and then
 * ask the rig where each of the eight tips ought to be.
 *
 * @param cx body centre x, @param cy body centre y, @param ts tile size in px
 * @param movingX normalised walk direction x, @param movingY the same for y
 */
...
/** How far a fully closed lid squashes the sclera; a sliver stays visible. */
const EYE_LID_CLOSE_RATIO = 0.92;
...
function drawEyes(
  ...
  for (const eye of EYES) {
    ...
    ctx.ellipse(
      ...
      ey,
      er,
      er * (1 - blinkAmt * EYE_LID_CLOSE_RATIO) * (1 + scream * EYE_SCREAM_OPEN_SCALE),
      0,
      0,
      TWO_PI,
    );
...
/** Peak opacity of the red furnace glow down the open throat. */
const MAW_SCREECH_GLOW_ALPHA = 0.68;
...
function drawMaw(
  ...
  if (state === 'attack_screech' && stateProgress > MAW_SCREECH_GLOW_THRESH) {
    ctx.save();
    ctx.globalAlpha =
      Math.min(stateProgress * MAW_SCREECH_GLOW_ALPHA_SCALE, 1.0) * MAW_SCREECH_GLOW_ALPHA;
...
/** Where the body centre sits inside the tile the spider stands on. */
...
// Leg index constants (matching positions in SPIDER_LEGS array)
...
/** Everything a single painted pose of the spider depends on. */
export interface GrotesqueSpiderPose {
  /** Clock the body, hair, legs and maw oscillate off, in seconds. */
  readonly time: number;
  /**
   * Clock the eyelids blink off, in seconds.
   *
   * Its own clock because the spit pose freezes the body at time zero while the
   * eyes keep working through the wind-up — the one place the two diverge.
   */
  readonly eyeTime: number;
  /** Normalised horizontal facing (-1 left, 0, +1 right). */
  readonly facingX: number;
  /** Normalised vertical facing (-1 up, 0, +1 down). */
  readonly facingY: number;
  readonly state: GrotesqueSpiderState;
  /** 0–1 progress within an attack state; ignored by idle and walk. */
 ...
 *
 * @param sx   Tile top-left x, in the target's coordinate space
 * @param sy   Tile top-left y, in the target's coordinate space
 * @param ts   Tile size in pixels
 * @param pose Which frame of which animation to paint
 */
export function drawGrotesqueSpider(
  ...
  const { time, eyeTime, facingX, facingY, state, stateProgress } = pose;
  const cx = sx + ts * SPIDER_BODY_CENTRE_RATIO;
  const cy = sy + ts * SPIDER_BODY_CENTRE_RATIO;
  ...
  for (const i of BACK_LEG_INDICES) {
    const leg = SPIDER_LEGS[i];
    const { tx, ty } = getSpiderLegTip(cx, cy, ts, leg, time, state, stateProgress, movX, movY);
  ...
  drawEyes(ctx, cx, cy, ts, eyeTime, drawFacingX, facingY, state, stateProgress, !movingUp);
