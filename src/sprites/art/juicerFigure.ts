/**
 * The Juicer, as a painted figure: the choreography, the cell geometry, and the
 ...
 * row table, the held-dumbbell anchor computation, and the placement of a pose
 * or a gore piece inside its cell. Anatomy, palette and every stroke of paint
 * live in `juicerArt.ts` and `juicerGore.ts`.
 *
 * The gait is Carl's, scaled to a much heavier creature: stance is a
 * constant-rate backward slide so the contact rolls rather than skates, swing
 * is a keyed tuck -> pass -> reach, and the pelvis *drops* at contact so the IK
 * never clamps. Arms swing from joint angles, not from hand targets — almost
 * all of a walking arm's travel belongs to the shoulder, and an arm placed by
 * its hand cannot do that. Hand targets are kept for the two one-shots, where
 * the hands are going somewhere specific.
 *
 * The art invariants live in `scripts/gates-juicer.ts`, which the review
 * harness runs: `npm run render:juicer`.
 */

import { clamp01, deg, easeInOut, lerp } from './carlArt';
import {
  ARM_ROOT_HALF,
  ELBOW_IN_FRONT,
  type ArmAngles,
...
import { type GorePiece, juicerGorePieces } from './juicerGore';
...
import type { JuicerHandView, TileFraction } from '../juicerHandAnchor';
 ...
 * widest pose plus padding, quantised — and `scripts/parity-figure-sheet.ts`
...
/**
 * How far the parts of him that merely hang trail the breath and sway that
 * drive them, in cycle turns.
 *
 * The tail and the slack arms are the two things about a posing bodybuilder he
 * is *not* holding: heavy masses dragged along by the hips and the lats rather
 * than placed with the chest, and a dragged mass arrives after its anchor does.
 * A quarter turn is the lag that reads — far enough that they are visibly still
 * finishing the last breath's swing, near enough that they never fight the
 * body.
 *
 * It is also what stops the idle being a single oscillator. Every other term of
 * the standing pose rides the breath's own sine, and a sine sampled either side
 * of a turning point paints the same picture twice: the eight-frame row showed
 * six pictures head-on and edge-on and five from behind, where there is no
 * blink to break the tie. The tail alone fixes the profile and the back, where
 * it is a quarter of the silhouette; head-on it is foreshortened to a stub
 * behind the thighs, and only the arms carry the difference there.
 */
const HANGING_DRAG_TURNS = 0.25;

/** The clock the hanging parts run on: the breath, a quarter turn late. */
function hangingDrag(phase: number): number {
  return Math.sin((phase - HANGING_DRAG_TURNS) * FULL_TURN);
}
...
function idleBase(phase: number): JuicerPose {
  ...
  pose.tailSwing = hangingDrag(phase) * IDLE_TAIL_SWAY;
  return pose;
}
...
function idleFacing(phase: number, away: boolean): JuicerPose {
  ...
  const armDrift = hangingDrag(phase) * IDLE_ARM_DRIFT;
  pose.sway = sway * IDLE_SWAY;
  pose.headTurn = sway * IDLE_HEAD_TURN;
  pose.headTilt = sway * IDLE_HEAD_TILT;
  pose.leftArmAngles = facingArmAngles(LEFT_ARM, armDrift, 0.5, FACING_ELBOW_FLEX);
  pose.rightArmAngles = facingArmAngles(RIGHT_ARM, armDrift, 0.5, FACING_ELBOW_FLEX);
 ...
 * long, and `FRAME_WIDTH` is a frozen number the cache allocates every warm
 * cell at. A wider swing does not grow the cell any more — it runs out of it,
 * and the structural gate fails the row for painting against the edge.
 */
...
/**
 * And no higher. `FRAME_HEIGHT` is frozen, so fists carried further up do not
 * make the cell taller — they leave it, and the structural gate fails the row
 * for painting against the edge. Growing the cell instead is not free either:
 * the whole row is warmed at once, and its bytes are what the warm-row budget
 * gate measures.
 */
...
export interface RowSpec {
  ...
  readonly pose: (frame: number) => JuicerPose;
}
...
export const JUICER_ROWS: readonly RowSpec[] = [
  {
    name: 'idle',
...
function paintView(ctx: CanvasRenderingContext2D, view: JuicerView, pose: JuicerPose): void {
...
export function handAnchorsOfRow(name: string): TileFraction[] {
  ...
  return Array.from({ length: row.frameCount }, (_unused, frame) =>
    tileFractionOf(gripHandOf(row.pose(frame), row.view)),
  );
}
 ...
 * `scripts/gates-juicer.ts` re-measures them on every render.
 */
export const GORE_RECENTRE: ReadonlyMap<string, Pt> = new Map([
  ['gore_head', pt(0.02297794117647059, -0.04595588235294118)],
  ['gore_torso', pt(0.013786764705882353, 0.04595588235294118)],
  ['gore_arm', pt(0.02297794117647059, -0.06433823529411765)],
  ['gore_forearm', pt(0.027573529411764705, 0.004595588235294118)],
  ['gore_thigh', pt(-0.03676470588235294, -0.06893382352941177)],
  ['gore_shin', pt(-0.01838235294117647, 0.08731617647058824)],
  ['gore_entrails', pt(-0.004595588235294118, 0)],
  ['gore_tail', pt(0.09650735294117647, 0.04595588235294118)],
]);
...
function poseRowOf(state: string): RowSpec | undefined {
  return JUICER_ROWS.find((row) => row.name === state);
}

/**
 * Paints one cell of the Juicer, in the cell's own pixels.
 *
 ...
 */
function paintJuicerFrame(ctx: CanvasRenderingContext2D, state: string, frame: number): void {
...
function juicerStateFrames(): Record<string, number> {
  const frames: Record<string, number> = {};
  for (const row of JUICER_ROWS) frames[row.name] = row.frameCount;
  for (const state of GORE_STATES) frames[state] = 1;
  return frames;
}

export const JUICER_FIGURE: FigureDef = {
  id: 'juicer',
  ...
  states: figureStates(juicerStateFrames()),
  paintFrame: paintJuicerFrame,
};
