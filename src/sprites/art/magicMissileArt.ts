...
/**
 * Where in its spin a projectile frame sits, in turns.
 *
 * Exported because the pixels cannot say how much of a turn the row covers: the
 * braid decorrelates completely within about a third of a turn, so past that a
 * wrap and an ordinary step measure the same saturated number and a row running
 * one and a half turns reads as an ordinary loop. The gate asserts the coverage
 * on this mapping, where the defect is unambiguous.
 */
export function projectilePhase(frame: number): number {
  return frame / PROJECTILE_FRAME_COUNT;
}
...
export function drawMagicMissileProjectile(
  ...
  const phase = projectilePhase(frame);
