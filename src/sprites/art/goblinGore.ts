...
/** How far apart two pieces' noise seeds sit, so no two share a grain. */
const PIECE_SEED_STRIDE = 977;
/**
...
export function gorePieces(style: GoblinStyle): readonly GorePiece[] {
  ...
  let seedCounter = PIECE_SEED_START;
  // Allocated here, in the order the pieces are painted in, so a piece's grain
  // is a property of the piece and not of when it was drawn.
  const nextSeed = (): number => PIECE_SEED_BASE + seedCounter++ * PIECE_SEED_STRIDE;
  const headWoundSeed = nextSeed();
  const torsoNeckWoundSeed = nextSeed();
  const torsoHipWoundSeed = nextSeed();
  const armNearWoundSeed = nextSeed();
  const armFarWoundSeed = nextSeed();
  const legNearWoundSeed = nextSeed();
  const legFarWoundSeed = nextSeed();
  ...
  interface LimbSpec {
    ...
    /** Noise seed for the cut face's grain, allocated with the other pieces'. */
    readonly woundSeed: number;
  }
  ...
  const entrails: GorePiece = {
    ...
    paint: (ctx) => {
      ...
      const noise = seededNoise(entrailsSeed);
      ctx.save();
