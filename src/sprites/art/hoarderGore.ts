...
import { TWO_PI, type Pt, deg, hash1, lerp, ovalOutline, traceOutline } from './ratArt';
import { BLOOD_DARK, drawWound, grownOutline, paintGoreMass } from './goreWound';
import {
  ANKLE_WIDTH,
  BELLY_HALF,
  ELBOW_WIDTH,
...
function torsoPiece(ctx: Ctx): void {
  ...
  TORSO_FOLD_SHARES.forEach((share, index) => {
    ...
    const span = TORSO_RX * TORSO_FOLD_SPANS[index];
    ctx.beginPath();
    ctx.moveTo(-span, y);
