...
import { AMBIENT_ALPHA, drawWound, grownOutline, paintGoreMass } from './goreWound';
import { TWO_PI, type Pt, deg, hash1, lerp, ovalOutline, rgba, traceOutline } from './ratArt';
...
function speckleHide(ctx: Ctx, trace: (grow: number) => void, seed: number, spread: number): void {
  // Composed with whatever alpha the caller is drawing at rather than replacing
  // it: a settled gore piece is faded out by its own system, and a painter that
  // assigns an absolute alpha paints straight through that fade.
  const baseAlpha = ctx.globalAlpha;
  ctx.save();
  trace(0);
  ctx.clip();
  ctx.globalAlpha = baseAlpha * SPECK_ALPHA;
