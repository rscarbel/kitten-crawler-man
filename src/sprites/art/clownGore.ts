...
/** How far the vented gas has got above the glass it is leaking out of. */
const PLUME_RISE = VIAL_CLUSTER_RADIUS * 0.85;
const PLUME_HALF_WIDTH = VIAL_CLUSTER_RADIUS * 0.5;
const PLUME_HALF_HEIGHT = VIAL_CLUSTER_RADIUS * 1.05;
...
function paintVials(ctx: Ctx, style: ClownStyle): void {
  ...
  // The escaping cloud sits behind the glass so the vials read as venting, and
  // it *rises* off them in a column rather than haloing them. Painted as a
  // circle centred on the glass, the plume was the whole silhouette of this
  // piece and it was a disc — which is also what the ruff is, so in a pile of
  // ten pieces the two were one shape: 82% of their outlines coincided, against
  // 62% for every other figure's gore in the game.
  ctx.save();
  ctx.translate(0, -PLUME_RISE);
  ctx.scale(PLUME_HALF_WIDTH / PLUME_HALF_HEIGHT, 1);
  const plume = ctx.createRadialGradient(0, 0, 0, 0, 0, PLUME_HALF_HEIGHT);
  plume.addColorStop(0, rgba(GAS_LIGHT, 0.5));
  plume.addColorStop(1, rgba(GAS_LIGHT, 0));
  ctx.fillStyle = plume;
  ctx.beginPath();
  ctx.arc(0, 0, PLUME_HALF_HEIGHT, 0, TWO_PI);
  ctx.fill();
  ctx.restore();
 ...
 *
 * `name` is not decoration: `clownFigure.ts` declares one figure state per
 * entry, and `EVIL_CLOWN_GORE_PARTS` in `evilClownSprite.ts` is the runtime
 * half of the same list. `scripts/gates-clowns.ts` holds the two together, so
 * a rename or a reorder here goes red instead of quietly spawning the wrong
 * piece.
 */
