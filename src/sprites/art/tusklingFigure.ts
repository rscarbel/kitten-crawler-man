/**
 * The Tuskling, as a painted figure: the choreography, the cell geometry, and
 ...
 *
 * The art invariants live in `scripts/gates-tuskling.ts`, which the review
 * harness runs: `npm run render:tuskling`.
 */
...
import { type GorePiece, tusklingGorePieces } from './tusklingGore';
...
export interface RowSpec {
  ...
  readonly pose: (frame: number) => TusklingPose;
}
...
/** The scale a gore piece's own units are painted at inside its cell. */
...
export const TUSKLING_ROWS: readonly RowSpec[] = [
  {
    name: 'idle',
...
function paintView(ctx: CanvasRenderingContext2D, view: TusklingView, pose: TusklingPose): void {
 ...
 * `scripts/gates-tuskling.ts` re-measures them on every render.
 */
export const GORE_RECENTRE: ReadonlyMap<string, Pt> = new Map([
  ['gore_head', pt(-0.005208333333333333, -0.036458333333333336)],
  ['gore_torso', pt(0.03125, 0.057291666666666664)],
  ['gore_arm', pt(0.005208333333333333, 0.020833333333333332)],
  ['gore_leg', pt(0.078125, -0.005208333333333333)],
  ['gore_ribcage', pt(-0.03125, 0.010416666666666666)],
  ['gore_entrails', pt(-0.020833333333333332, -0.005208333333333333)],
  ['gore_tusk', pt(0.08854166666666667, -0.036458333333333336)],
  ['gore_jaw', pt(0.005208333333333333, 0.026041666666666668)],
]);
...
function poseRowOf(state: string): RowSpec | undefined {
  return TUSKLING_ROWS.find((row) => row.name === state);
}

/**
 * Paints one cell of the Tuskling, in the cell's own pixels.
 *
 ...
 */
function paintTusklingFrame(ctx: CanvasRenderingContext2D, state: string, frame: number): void {
...
function tusklingStateFrames(): Record<string, number> {
  const frames: Record<string, number> = {};
  for (const row of TUSKLING_ROWS) frames[row.name] = row.frameCount;
  for (const state of GORE_STATES) frames[state] = 1;
  return frames;
}

export const TUSKLING_FIGURE: FigureDef = {
  id: 'tuskling',
  ...
  states: figureStates(tusklingStateFrames()),
  paintFrame: paintTusklingFrame,
};
