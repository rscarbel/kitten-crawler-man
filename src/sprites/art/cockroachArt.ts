...
/**
 * The smallest distance, in tiles, from the body's own centre to the furthest
 * ink on every skitter frame — which is an antenna tip, on every view.
 *
 * Declared here rather than in the gate so the number the art promises and the
 * number the gate enforces cannot drift apart. It is a *reach* rather than the
 * silhouette's bounding span it used to be, because the span is carried by the
 * body and the legs as much as by the antennae: shortening these filaments by a
 * quarter moved that measurement by a single percent and left the gate green,
 * while the reach tracks their length almost proportionally.
 */
export const ANTENNA_MIN_REACH_TILES = 0.92;
