 ...
 *
 * Every painter here that lowers `globalAlpha` composes with the value it was
 * handed — `const baseAlpha = ctx.globalAlpha` at the top, multiply into it,
 * and put that value back rather than 1. A caller draws these same shapes
 * through a fade, and an absolute alpha paints straight through it: a settled
 * gore piece stops fading out and sits on the floor at full strength.
 */
...
function drawUpperArmMuscle(
  ...
  ctx.globalAlpha = baseAlpha * tricepsAlpha;
  ...
  ctx.globalAlpha = baseAlpha * (fromBehind ? TRICEPS_ALPHA : BICEPS_ALPHA);
  ...
  if (!fromBehind) {
    ...
    ctx.globalAlpha = baseAlpha * TRICEPS_CROWN_ALPHA;
    fillEllipse(
      ctx,
...
function speckleTorso(ctx: Ctx, skeleton: Skeleton, spread: number): void {
  const baseAlpha = ctx.globalAlpha;
  ctx.globalAlpha = baseAlpha * SCALE_SPECK_ALPHA;
...
function drawBack(ctx: Ctx, skeleton: Skeleton, half: number, torsoHeight: number): void {
  ...
  ctx.globalAlpha = baseAlpha * TRAP_ALPHA;
  ...
  for (const side of [-1, 1]) {
    ...
    ctx.globalAlpha = baseAlpha * LAT_ALPHA;
...
function drawProfileFlank(ctx: Ctx, skeleton: Skeleton, half: number, torsoHeight: number): void {
  const baseAlpha = ctx.globalAlpha;
  ctx.globalAlpha = baseAlpha * FLANK_SHADE_ALPHA;
  fillEllipse(
    ctx,
...
function drawTorso(ctx: Ctx, skeleton: Skeleton, pose: JuicerPose, view: ViewSpec): void {
  ...
  ctx.globalAlpha = baseAlpha * MASS_SHADE_ALPHA;
  fillEllipse(ctx, shadeCentre, half, torsoHeight * 0.62, HIDE.dark);
  ctx.globalAlpha = baseAlpha * MASS_LIGHT_ALPHA;
  fillEllipse(
    ctx,
  ...
  if (view.profile) {
    ...
    ctx.globalAlpha = baseAlpha * BELLY_ALPHA;
    fillEllipse(
      ctx,
