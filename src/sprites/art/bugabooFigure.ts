/**
 * The Bugaboo, as a painted figure: the choreography, the cell geometry, and
 * the `FigureDef` the runtime cache and the review harness both draw through.
 *
 * The seven-foot, obsidian-haired thing that comes up through the floor grates,
 * and the shape Mordecai wears on level 2. Anatomy and every stroke of paint
 * live in `bugabooArt.ts`; this module is one pose function per animation row,
 * sampled per frame, plus where that pose sits inside its cell.
 *
 * Rows:
 *    idle          — toward the camera, breathing, coat shifting
 *    idle_side     — profile, drawn facing +X and mirrored at runtime
 *    idle_away     — away from the camera
 *    walk, walk_side, walk_away
 *    swipe         — a two-clawed rake at something in front of it
 *    swipe_side, swipe_away
 *    breach        — half out of a smashed floor, groping around the hole
 *    emerge        — hauling itself the rest of the way out
 *
 ...
 *
 * The art invariants live in `scripts/gates-bugaboo.ts`, which the review
 * harness runs: `npm run render:bugaboo`.
 */

import { clamp01, deg, easeInOut, lerp, ramp } from './carlArt';
import {
  ARM_ROOT_HALF,
  FOOT_STAND_SPREAD,
  FOREARM_LENGTH,
...
export const BUGABOO_ROWS: readonly RowSpec[] = [
  {
    name: 'idle',
...
function paintView(ctx: CanvasRenderingContext2D, view: BugabooView, pose: BugabooPose): void {
...
function poseRowOf(state: string): RowSpec {
  const row = BUGABOO_ROWS.find((candidate) => candidate.name === state);
  if (row === undefined) throw new Error(`no bugaboo row for "${state}"`);
  return row;
}

/** Paints one cell of the Bugaboo, in the cell's own pixels. */
function paintBugabooFrame(ctx: CanvasRenderingContext2D, state: string, frame: number): void {
...
function bugabooStateFrames(): Record<string, number> {
  const frames: Record<string, number> = {};
  for (const row of BUGABOO_ROWS) frames[row.name] = row.frameCount;
  return frames;
}

export const BUGABOO_FIGURE: FigureDef = {
  id: 'bugaboo',
  ...
  states: figureStates(bugabooStateFrames()),
  paintFrame: paintBugabooFrame,
};
