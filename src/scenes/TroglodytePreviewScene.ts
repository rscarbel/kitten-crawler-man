/**
 * Localhost-only harness for watching the troglodyte move, gape and lash.
 *
 * Reached via `?trog` in `devBootScene`; never on a production path. It exists
 * because stills genuinely cannot answer the questions that matter about this
 * creature: `scripts/render-troglodyte.ts` can show that the walk loop closes
 * and the jaw hinges, but only playback shows whether the fifty-frame windup
 * reads as a warning in the time the player actually has to sidestep it, and
 * whether the tongue leaves the mouth where the mouth is.
 *
 * All four facings play every row simultaneously on a labelled grid, over a
 * backdrop that cycles the real floor palettes so the contrast check happens
 * against the thing the creature actually stands on. "kill" fires the real
 * `BodyPartGoreSystem`, which is the only way to check that pieces tumble in
...
type RowKind = 'idle' | 'walk' | 'gape' | 'lash';
...
interface RowSpec {
   ...
   * `Troglodyte` drives them with, so the pacing seen here is the pacing the
   * player gets rather than a second guess at it.
   */
  readonly gameFrames: number;
}

/** Matches `WINDUP_FRAMES` and `STRIKE_FRAMES` in `src/creatures/Troglodyte.ts`. */
const WINDUP_GAME_FRAMES = 50;
const STRIKE_GAME_FRAMES = 18;
const IDLE_GAME_FRAMES = 60;
const WALK_GAME_FRAMES = 44;

const ROWS: ReadonlyArray<RowSpec> = [
  { kind: 'idle', probeState: 'idle_side', gameFrames: IDLE_GAME_FRAMES },
  { kind: 'walk', probeState: 'walk_side', gameFrames: WALK_GAME_FRAMES },
  { kind: 'gape', probeState: 'gape_side', gameFrames: WINDUP_GAME_FRAMES },
...
/** The floor mids a troglodyte actually stands on, from `scripts/tilegen/palette.ts`. */
...
/**
 * Cells are wide because the tongue is not: it reaches three tiles, and a cell
 * sized for the creature would clip the only attack it has.
 */
const CELL_WIDTH_TILES = 4.5;
const CELL_HEIGHT_TILES = 3;
...
/** Where in the strike the tongue is at full reach, per `Troglodyte.updateAI`. */
const TONGUE_PEAK_AT = 0.5;

/**
 * The strike's own tongue curve, matching `Troglodyte.updateAI`: out over the
 * first half of the strike and reeled back over the second.
 */
function tongueExtendAt(strikeProgress: number): number {
  const halves = 1 / TONGUE_PEAK_AT;
  return strikeProgress <= TONGUE_PEAK_AT ? strikeProgress * halves : (1 - strikeProgress) * halves;
}

export class TroglodytePreviewScene extends Scene {
      ...
      VIEWS.forEach((view, column) => {
        ...
        drawTroglodyteSprite(
          ...
          tile,
          {
            ...
            gapeProgress: row.kind === 'gape' ? progress : 0,
            strikeProgress: row.kind === 'lash' ? progress : null,
            tongueExtend: row.kind === 'lash' ? tongueExtendAt(progress) : 0,
            ...
            idleFrame:
              row.kind === 'idle' ? Math.floor(progress * frameCountOf(row.probeState)) : undefined,
          },
        );
        ctx.restore();
  ...
  private renderHeader(ctx: CanvasRenderingContext2D, width: number): void {
    drawText(ctx, 'troglodyte preview — ?trog', {
      x: MARGIN,
      y: MARGIN + TITLE_SIZE,
      size: TITLE_SIZE,
      color: '#fdfaf2',
