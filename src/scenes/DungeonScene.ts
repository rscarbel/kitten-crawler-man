...
export class DungeonScene extends Scene {
  ...
  /** True once either player has entered the safe room (gates safe_haven achievement). */
  ...
  /** Cached rect for the achievement icon button (rebuilt each render). */
  private _achievIconRect = { x: 0, y: 0, w: 80, h: 28 };
  /** Cached rect for the flashing loot box icon in safe room. */
  ...
  /** Ordered queue of notifications to show. Drained one-by-one as user clicks OK. */
  private _notifQueue: Array<{ def: AchievementDef; mgr: AchievementManager }> =
    [];

  // ── Health regeneration ────────────────────────────────────────────────────
  /** Fractional HP accumulator for human regen (prevents precision loss on small rates). */
  private humanRegenAccum = 0;
  /** Fractional HP accumulator for cat regen. */
  private catRegenAccum = 0;
  /** Human regenerates to full HP over 3 minutes (10800 frames at 60 fps). */
  private readonly HUMAN_REGEN_FRAMES = 10800;
  /** Cat regenerates to full HP over 4 minutes (14400 frames at 60 fps). */
  private readonly CAT_REGEN_FRAMES = 14400;
  ...
  handleClick(mx: number, my: number): void {
    // ── Loot box opener absorbs all clicks while running ───────────────────
    if (this.lootBoxOpener.isOpen) {
      return;
    }

    // ── Achievement notification overlay (cycles through unread) ──────────
    if (this._notifActive) {
      if (this.achievementNotif.handleClick(mx, my)) {
        // Remove this notification from its manager's pending list
    ...
    // ── Achievement icon — only visible when there are unread notifications ─
    if (!this.gameOver && !this.pauseMenu.isOpen) {
      ...
      if (mx >= ai.x && mx <= ai.x + ai.w && my >= ai.y && my <= ai.y + ai.h) {
        ...
        if (totalUnread > 0) {
          // Build the queue from both managers in order
          this._notifQueue = [
  ...
  update(): void {
    // Always animate overlays regardless of pause/sleep state
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    // ── Achievement notification overlay (top layer) ──────────────────────
  ...
  private updateGameplay(): void {
    ...
    // ── Safe Haven achievement (first entry) ───────────────────────────────
    if (
  ...
  private updateSleep(): void {
    ...
    if (this.sleepTimer <= 0) {
      ...
      // Sleeping costs 3 minutes off the level timer
      this.levelTimerFrames = Math.max(0, this.levelTimerFrames - 10800);
    }
  }
  ...
  private resolveKills() {
    for (const mob of this.mobs) {
      ...
      // ── Identify top damage dealer ───────────────────────────────────────
      let topPlayer: HumanPlayer | CatPlayer | null = null;
      let otherPlayer: HumanPlayer | CatPlayer | null = null;
      let maxDmg = 0;
      for (const [player, dmg] of mob.damageTakenBy) {
        if (dmg > maxDmg) {
          maxDmg = dmg;
          topPlayer = player as HumanPlayer | CatPlayer;
        }
      }
      otherPlayer = topPlayer === this.human ? this.cat : this.human;

      // ── Award XP: 85% to top dealer, 15% to the other party member ──────
      ...
      // ── Achievement checks ───────────────────────────────────────────────
      // first_blood goes to the killer
  ...
  private openBoxQueue(player: 'human' | 'cat'): void {
    ...
    this.lootBoxOpener.startQueue(
      ...
      (box, contents) => {
        ...
        if (contents.bonus) {
          // Bonus items (e.g. dynamite) go to Human regardless
      ...
      () => {
        // All boxes opened — nothing extra needed
      },
    );
  }
  ...
  private achievementIconRect() {
    const mmSize = this.miniMapExpanded
      ...
      : this.MM_NORMAL_SIZE;
    // Sits directly below the pause button
    return {
  ...
  private drawAchievementIcon(ctx: CanvasRenderingContext2D): void {
    if (
      ...
      // Hide the button when there's nothing new or the notif overlay is open
    ...
    // Only show the icon when there are unread achievements
    if (unread === 0) {
      this._achievIconRect = { x: -9999, y: 0, w: 0, h: 0 };
      return;
    }
