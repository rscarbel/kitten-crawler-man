...
All sprite sheets in this directory are generated by `scripts/generateSprites.ts`.
To regenerate: `tsx scripts/generateSprites.ts`

The manifest (`manifest.json`) is rewritten on every generation run.
...
```bash
tsx scripts/generateSprites.ts
```

This overwrites all PNGs and rewrites `manifest.json`. The script lives at `scripts/generateSprites.ts` and imports sprite drawing functions directly from `src/sprites/*.ts`. Do not hand-edit `manifest.json`; it is a build artifact.
