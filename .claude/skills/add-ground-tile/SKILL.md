...
> **Why generated:** tileability is a *mathematical* property — the pixels at
...
Wrapping makes a patch seamless against *itself*, **not** against a
...
with `variant` hashed from the *patch* coordinates (`floor(tx / n)`,
...
*per patch phase*, and would fix at build time which pairs may blend. Composite at
load instead:
