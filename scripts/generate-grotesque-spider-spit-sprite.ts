...
function drawBody(ctx: NodeCtx, cx: number, cy: number, ts: number, time: number): void {
  ...
  ctx.beginPath();
  ctx.ellipse(cx, cy - ts * 0.05, ts * (0.74 + breathe), ts * (0.6 + breathe), 0, 0, TWO_PI);
  ctx.fill();
...
function drawEyes(ctx: NodeCtx, cx: number, cy: number, ts: number, time: number): void {
  for (const eye of EYES) {
    ...
    ctx.beginPath();
    ctx.ellipse(ex, ey, er, er * (1 - blinkAmt * 0.92), 0, 0, TWO_PI);
    ctx.fill();
    if (eye.bloodshot) {
      ctx.save();
    ...
    if (blinkAmt < 0.5) {
      ...
      if (eye.slit) {
        ctx.beginPath();
        ctx.ellipse(ex, ey, pupilR, er * 0.56, 0, 0, TWO_PI);
        ctx.fill();
      } else {
        ctx.beginPath();
        ctx.arc(ex, ey, pupilR, 0, TWO_PI);
        ctx.fill();
      }
...
function drawCreatureSpit(
  ...
  if (leanY !== 0) {
    ctx.save();
    ctx.translate(0, leanY);
  }
  drawHair(ctx, cx, cy, ts, 0);
  for (const i of [2, 3, 6, 7]) {
    const leg = LEGS[i];
    const { tx, ty } = getIdleLegTip(cx, cy, ts, leg);
...
function drawSpitTrapIdle(ctx: NodeCtx, cx: number, cy: number, ts: number, frame: number): void {
  ...
  for (const strand of WEB_STRANDS) {
    ...
    const bx2 = cx + Math.cos(strand.bAngle) * rx * strand.rB;
    const by2 = cy + Math.sin(strand.bAngle) * ry * strand.rB;
    const cpx = (ax + bx2) / 2 + sway;
    const cpy = (ay + by2) / 2 + sway2;
    ctx.beginPath();
    ctx.moveTo(ax, ay);
  ...
  if (bubbleAlpha > 0.05) {
    ...
    ctx.lineWidth = 1.3;
    ctx.beginPath();
    ctx.arc(cx + ts * 0.32, cy - ts * 0.12, bubbleR, 0, TWO_PI);
    ctx.stroke();
    ctx.globalAlpha = bubbleAlpha * 0.38;
    ctx.fillStyle = '#aace24';
    ctx.beginPath();
    ctx.arc(
      cx + ts * 0.32 - bubbleR * 0.32,
      cy - ts * 0.12 - bubbleR * 0.32,
      bubbleR * 0.24,
      0,
      TWO_PI,
    );
  ...
  if (b2a > 0.05) {
    ctx.globalAlpha = b2a * 0.48;
    ctx.strokeStyle = '#7a9a14';
    ctx.lineWidth = 1.0;
    ctx.beginPath();
    ctx.arc(cx - ts * 0.38, cy + ts * 0.18, ts * 0.058 * b2a, 0, TWO_PI);
    ctx.stroke();
  }
  ctx.restore();
}
...
function renderSheet(rowGroups: Array<Array<() => void>>, frameW: number, frameH: number): Buffer {
...
// Re-do properly with the shared canvas
{
  ...
  for (let col = 0; col < ATK_FRAMES; col++) {
    ctx.save();
    ctx.beginPath();
    ctx.rect(col * SPIT_FRAME_W, 0, SPIT_FRAME_W, SPIT_FRAME_H);
    ctx.clip();
    drawCreatureSpit(
      ctx,
      col * SPIT_FRAME_W + SPIT_TILE_X,
      SPIT_TILE_Y,
      SPIT_TILE_SCALE,
      spitSteps[col],
    );
...
console.log('  [2/3] grotesque_spider_spit_projectile.png  (8-frame wobble) …');
{
  ...
  for (let col = 0; col < PROJ_FRAMES; col++) {
    ctx.save();
    ctx.beginPath();
    ctx.rect(col * PROJ_FRAME_W, 0, PROJ_FRAME_W, PROJ_FRAME_H);
    ctx.clip();
...
console.log('  [3/3] grotesque_spider_spit_trap.png  (8-frame splat + 8-frame idle) …');
{
  ...
  for (let col = 0; col < TRAP_FRAMES; col++) {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(col * TRAP_FRAME_W, 0, TRAP_FRAME_W, TRAP_FRAME_H);
    ctx.clip();
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(col * TRAP_FRAME_W, TRAP_FRAME_H, TRAP_FRAME_W, TRAP_FRAME_H);
    ctx.clip();
    drawSpitTrapIdle(
      ctx,
      col * TRAP_FRAME_W + TRAP_TILE_X,
      TRAP_FRAME_H + TRAP_TILE_Y,
      TRAP_TILE_SCALE,
      col,
    );
    ctx.restore();
...
console.log(
  `  grotesque_spider_spit:            frameWidth=${SPIT_FRAME_W}  frameHeight=${SPIT_FRAME_H}  tileX=${SPIT_TILE_X}  tileY=${SPIT_TILE_Y}  tileScale=${SPIT_TILE_SCALE}`,
);
console.log(
  `  grotesque_spider_spit_projectile: frameWidth=${PROJ_FRAME_W}  frameHeight=${PROJ_FRAME_H}  tileX=${PROJ_TILE_X}  tileY=${PROJ_TILE_Y}  tileScale=${PROJ_TILE_SCALE}`,
);
console.log(
  `  grotesque_spider_spit_trap:       frameWidth=${TRAP_FRAME_W}  frameHeight=${TRAP_FRAME_H}  tileX=${TRAP_TILE_X}  tileY=${TRAP_TILE_Y}  tileScale=${TRAP_TILE_SCALE}`,
);
