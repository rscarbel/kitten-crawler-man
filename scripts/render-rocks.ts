/**
 * Headless review harness for the Level 3 wilderness's boulders.
 *
 ...
 * draws both:
 *
 *   npx tsx scripts/render-rocks.ts --out=rocks.png --scale=3
 ...
 *
 * Regenerate the sheets with `npm run gen:rocks`.
 */

import { createCanvas, loadImage, type Image } from 'canvas';
import { resolve } from 'node:path';
import manifest from '../src/images/environment/rocks/manifest.json';
...
const DEFAULT_OUT = `${PREVIEW_DIR}/rocks.png`;
...
/** In-game strip: how many tiles of ground each rock is shown standing on. */
const STRIP_TILES_PER_ROCK = 4;
...
sheets.forEach(([key, geometry], index) => {
  ...
  // A patch of ground behind the rock, so the contact shadow and the moss are
  // judged against something the rock will actually stand on rather than
  ...
  // The anchor tile: the one tile the rock actually blocks, and the line it
  // Y-sorts against.
...
sheets.forEach(([key, geometry], index) => {
  ...
  const anchorLeft = index * STRIP_TILES_PER_ROCK * IN_GAME_TILE + IN_GAME_TILE;
...
console.log(`${writtenPath}: ${sheets.length} rock sheet(s) at ${scale}x`);
