...
function h(a: number, b: number, c = 0): number {
  return (
    ((Math.imul(a, 374761393) ^ Math.imul(b, 1103515245) ^ Math.imul(c, 668265263)) >>> 0) % 65536
  );
}
const hf = (a: number, b: number, c = 0): number => h(a, b, c) / 65535;
const clamp = (v: number): number => Math.max(0, Math.min(255, Math.round(v)));
const rgb = (r: number, g: number, b: number): string => `rgb(${clamp(r)},${clamp(g)},${clamp(b)})`;
...
interface FPalette {
  sr: number;
  sg: number;
  sb: number; // stone base (light grey family)
  cr: number;
  cg: number;
  cb: number; // seam colour
...
function drawFloorBase(
  ...
  for (let qi = 0; qi < quads.length; qi++) {
    ...
    const accentShift =
      hasAccent && qi === accentIdx ? (h(variant, 5, 200) % 2 === 0 ? 12 : -10) : 0;
...
function drawMossOverlay(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const mx = nearV
      ? seam.vx - 3 + Math.floor(hf(variant, i, 72) * 6)
      : Math.floor(hf(variant, i, 72) * T);
    const my = nearV
      ? Math.floor(hf(variant, i, 73) * T)
      : seam.hy - 3 + Math.floor(hf(variant, i, 73) * 6);
    const mr = 1 + Math.floor(hf(variant, i, 74) * 3);
    ctx.fillStyle = rgba(30, 58, 30, 0.14 + hf(variant, i, 75) * 0.1);
    ctx.beginPath();
...
function drawOrnatePattern(
  ...
  switch (variant % 4) {
    case 0: {
      ctx.beginPath();
      ctx.moveTo(cx, oy + 10);
      ctx.lineTo(ox + T - 10, cy);
      ctx.lineTo(cx, oy + T - 10);
      ctx.lineTo(ox + 10, cy);
      ctx.closePath();
      ctx.stroke();
    ...
    case 2: {
      ...
      ctx.beginPath();
      ctx.moveTo(cx, oy + 8);
      ctx.lineTo(cx, oy + T - 8);
      ctx.moveTo(ox + 8, cy);
      ctx.lineTo(ox + T - 8, cy);
      ctx.stroke();
      ctx.beginPath();
    ...
    default: {
      ...
      for (let p = 0; p < 6; p++) {
        ...
        ctx.beginPath();
        ctx.arc(cx + Math.cos(a) * 20, cy + Math.sin(a) * 20, 2, 0, Math.PI * 2);
        ctx.fill();
      }
      break;
    }
...
function drawFloorTile(
  ...
  switch (state) {
    ...
    case 'floor_ornate': {
      ...
      ctx.fillStyle = rgb(
        FP.ornate.sr + FP.ornate.litBoost + off,
        FP.ornate.sg + FP.ornate.litBoost * 0.9 + off,
        FP.ornate.sb + FP.ornate.litBoost * 0.7 + off,
      );
...
function drawWallMossOverlay(
  ...
  ctx.fillStyle = rgba(16, 40, 14, 0.3);
...
writeFileSync(
  outPath,
  (canvas as unknown as { toBuffer: (fmt: string) => Buffer }).toBuffer('image/png'),
);
