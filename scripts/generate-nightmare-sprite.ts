 ...
 *
 * Run: npx tsx scripts/generate-nightmare-sprite.ts
 */
...
// ── Inline nightmare drawing code ─────────────────────────────────────────────
// (Inlined here so this script runs in Node via tsx without needing DOM types.)

type NightmareState = 'idle' | 'walk' | 'attack_slam' | 'attack_screech';
...
function drawHair(
  ctx: NodeCtx, cx: number, cy: number, ts: number, time: number,
  state: NightmareState, stateProgress: number, movingX: number, movingY: number,
): void {
...
function drawLeg(
  ctx: NodeCtx, ax: number, ay: number, tx: number, ty: number,
  ts: number, kOut: number,
): void {
  ...
  ctx.strokeStyle = '#1a0a12'; ctx.lineWidth = baseThick * 2.0;
  ...
  ctx.beginPath(); ctx.arc(kx, ky, baseThick * 1.3, 0, TWO_PI); ctx.fill();
...
function getLegTip(
  cx: number, cy: number, ts: number, leg: LegDesc, time: number,
  state: NightmareState, stateProgress: number, movingX: number, movingY: number,
): { tx: number; ty: number } {
  const restX = cx + leg.rx * ts; const restY = cy + leg.ry * ts;
  ...
  if (state === 'walk') {
    ...
    if (cycle > 0.25) {
      ...
      return { tx: restX + movingX * ts * 0.45 * liftT, ty: restY + movingY * ts * 0.25 * liftT - lift };
    }
    return { tx: restX, ty: restY };
  }
  if (state === 'attack_slam' && leg.ry < 0) {
    const lift = stateProgress < 0.55
...
function drawBody(
  ctx: NodeCtx, cx: number, cy: number, ts: number, time: number,
  state: NightmareState, stateProgress: number,
): void {
  ...
  ctx.beginPath();
  ctx.ellipse(cx, cy - ts * 0.05,
    ts * (0.74 + breathe + screamBulge * 0.28),
    ts * (0.60 + breathe + screamBulge * 0.28),
    0, 0, TWO_PI);
  ctx.fill();

  ctx.fillStyle = '#1b0b15';
  ctx.beginPath(); ctx.ellipse(cx - ts * 0.48, cy - ts * 0.26, ts * 0.43, ts * 0.37, -0.28, 0, TWO_PI); ctx.fill();

  ctx.fillStyle = '#170910';
  ctx.beginPath(); ctx.ellipse(cx + ts * 0.36, cy - ts * 0.10, ts * 0.34, ts * 0.46, 0.22, 0, TWO_PI); ctx.fill();

  ctx.fillStyle = '#1e0c18';
  ctx.beginPath(); ctx.ellipse(cx - ts * 0.09, cy + ts * 0.23, ts * 0.53, ts * 0.34, 0.14, 0, TWO_PI); ctx.fill();

  ctx.fillStyle = '#100609';
  ctx.beginPath(); ctx.ellipse(cx + ts * 0.05, cy - ts * 0.46, ts * 0.40, ts * 0.27, 0, 0, TWO_PI); ctx.fill();

  const tears: Array<readonly [number, number, number, number, number]> = [
    [-0.26, -0.05, 0.21, 0.077, 0.30],
    [ 0.19, -0.16, 0.13, 0.055, -0.22],
    [-0.05,  0.19, 0.11, 0.048,  0.78],
    [ 0.29,  0.09, 0.087, 0.038, -0.50],
  ];
  for (const [ox, oy, ew, eh, angle] of tears) {
    ctx.fillStyle = '#4a1530';
    ctx.beginPath(); ctx.ellipse(cx + ox * ts, cy + oy * ts, ew * ts, eh * ts, angle, 0, TWO_PI); ctx.fill();
    ctx.fillStyle = '#7a2040';
    ctx.beginPath();
    ctx.ellipse(cx + ox * ts - ts * ew * 0.22, cy + oy * ts - ts * eh * 0.28, ew * ts * 0.52, eh * ts * 0.40, angle, 0, TWO_PI);
    ctx.fill();
  }

  ctx.save(); ctx.globalAlpha = 0.12 + 0.07 * Math.sin(time * 2.05);
  ctx.strokeStyle = '#1e3612'; ctx.lineWidth = 1.4;
  const veins: Array<readonly [number, number, number, number, number, number]> = [
    [-0.31, -0.09,  0.09, -0.40, -0.16, 0.16],
    [ 0.14, -0.01, -0.11, -0.31,  0.19, -0.21],
    [-0.10,  0.21,  0.19,  0.10, -0.22, -0.10],
  ];
  ...
  if (state === 'attack_slam' && stateProgress > 0.82) {
    ...
    ctx.save(); ctx.globalAlpha = flashAlpha; ctx.fillStyle = '#fffaf0';
    ctx.beginPath(); ctx.ellipse(cx, cy + ts * 0.3, ts * 1.45, ts * 0.7, 0, 0, TWO_PI); ctx.fill();
    ctx.restore();
  }
}
...
function drawEyes(
  ...
  for (const eye of EYES) {
    ...
    if (blinkAmt < 0.5) {
      ...
      const pupilX = ex + facingX * er * 0.22; const pupilY = ey + facingY * er * 0.22;
      ctx.fillStyle = '#040204';
      if (eye.slit) {
        ctx.beginPath(); ctx.ellipse(pupilX, pupilY, pupilR, er * 0.56, 0, 0, TWO_PI); ctx.fill();
      } else {
        ctx.beginPath(); ctx.arc(pupilX, pupilY, pupilR, 0, TWO_PI); ctx.fill();
      }
...
function drawMaw(
  ...
  if (state === 'attack_screech') {
    openAmt = stateProgress < 0.5
      ? 0.055 + stateProgress * 2.0 * 0.82
      : 0.055 + 0.82 - (stateProgress - 0.5) * 0.5;
  } else if (state === 'attack_slam') { openAmt += stateProgress * 0.28; }

  const mawW = ts * (0.46 + openAmt * 0.28); const mawH = ts * openAmt;
  ctx.fillStyle = '#060003';
  ctx.beginPath(); ctx.ellipse(mawCx, mawCy, mawW, mawH, 0, 0, TWO_PI); ctx.fill();

  if (state === 'attack_screech' && stateProgress > 0.08) {
    ctx.save(); ctx.globalAlpha = Math.min(stateProgress * 2.0, 1.0) * 0.68;
    const grad = ctx.createRadialGradient(mawCx, mawCy, 0, mawCx, mawCy, mawW * 0.8);
    grad.addColorStop(0, '#ff1020'); grad.addColorStop(0.5, '#8b0012');
    grad.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = grad;
    ctx.beginPath(); ctx.ellipse(mawCx, mawCy, mawW * 0.9, mawH * 0.9, 0, 0, TWO_PI); ctx.fill();
    ctx.restore();
  }
...
function drawBackRidge(ctx: NodeCtx, cx: number, cy: number, ts: number, time: number): void {
  const ridgeX = cx + ts * 0.06; const ridgeTop = cy - ts * 0.52; const ridgeBot = cy + ts * 0.35;
  const count = 7;
  for (let i = 0; i < count; i++) {
    ...
    ctx.beginPath(); ctx.ellipse(ridgeX + pulse, ry, knobR * 1.4, knobR, 0.2, 0, TWO_PI); ctx.fill();
    ctx.fillStyle = '#3a1a28';
    ctx.beginPath(); ctx.arc(ridgeX + pulse - knobR * 0.2, ry - knobR * 0.3, knobR * 0.5, 0, TWO_PI); ctx.fill();
  }
}
...
function drawNightmare(
  ctx: NodeCtx, sx: number, sy: number, ts: number, time: number,
  facingX: number, facingY: number,
  state: NightmareState, stateProgress: number,
): void {
  const cx = sx + ts * 0.5; const cy = sy + ts * 0.5;
  const absX = Math.abs(facingX); const absY = Math.abs(facingY);
  ...
  if (needsFlip) { ctx.save(); ctx.translate(cx, 0); ctx.scale(-1, 1); ctx.translate(-cx, 0); }
  ...
  if (state === 'attack_screech' && stateProgress > 0.3) {
    ...
    ctx.save(); ctx.globalAlpha = (1 - ringProgress) * 0.5;
    ctx.strokeStyle = '#600010'; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.arc(cx, cy - ts * 0.1, ts * 1.6 * ringProgress, 0, TWO_PI); ctx.stroke();
    ctx.restore();
  }
...
type FrameSpec = {
  time: number; facingX: number; facingY: number;
  state: NightmareState; stateProgress: number;
};
...
const WALK_TIMES = [0.0, 0.38, 0.76, 1.14, 1.52, 1.90, 2.28, 2.66];
...
const BASE_ROWS: RowSpec[] = [
  {
    label: 'idle',
    frames: IDLE_TIMES.map(t => ({ time: t, facingX: 0, facingY: 1, state: 'idle', stateProgress: 0 })),
  },
  {
    label: 'walk_down',
    frames: WALK_TIMES.map(t => ({ time: t, facingX: 0, facingY: 1, state: 'walk', stateProgress: 0 })),
  },
  {
    label: 'walk_up',
    frames: WALK_TIMES.map(t => ({ time: t, facingX: 0, facingY: -1, state: 'walk', stateProgress: 0 })),
  ...
  {
    label: 'walk_side',
    frames: WALK_TIMES.map(t => ({ time: t, facingX: 1, facingY: 0, state: 'walk', stateProgress: 0 })),
  },
];

const SLAM_ROW: FrameSpec[] = ATK_STEPS.map(p => ({
  time: 0, facingX: 0, facingY: 1, state: 'attack_slam', stateProgress: p,
}));

const SCREECH_ROW: FrameSpec[] = ATK_STEPS.map(p => ({
  time: 0, facingX: 0, facingY: 1, state: 'attack_screech', stateProgress: p,
}));
  ...
  for (let row = 0; row < rowGroups.length; row++) {
    for (let col = 0; col < rowGroups[row].length; col++) {
      ...
      drawNightmare(
        ...
        row * FRAME_H + TILE_Y,
        TILE_SCALE,
        spec.time, spec.facingX, spec.facingY,
        spec.state, spec.stateProgress,
      );
    }
  }
...
console.log(`Generating grotesque_spider sprites (${FRAME_W}×${FRAME_H}px frames, tileScale=${TILE_SCALE})…`);
