...
`as` casts, no `!` assertions, no magic numbers, comments explain *why*, validation gates).
...
3. **Other agents are editing this repo right now** (status-effect visuals in
   ...
   you *must* edit (`src/scenes/DungeonScene.ts`, `src/dev/devBoot.ts`,
...
4. **Line numbers in this plan are a snapshot** taken 2026-08-02. Concurrent edits will
   ...
   number.
5. **`src/images/enemies/manifest.json` is shared.** The generator must *verify* its
...
7. **Every phase gets an independent review** (see §2 protocol). Per project memory:
   *a fix can entrench the bug it fixed* — after fixing review findings, always run one
...
of the feathering), and a fan at the end of his tail. The goal is *very realistic and
convincing as a dinosaur* — the current sheet fails at this and is being fully replaced.
...
**Image review protocol (art phases).** Reviewer agents must actually *look at* rendered
...
- If the **cat dies** while Mongo is out: Mongo runs to the cat and unsummons exactly
  like the 0-HP recall. Decision **(tunable)**: his HP is *preserved* (not zeroed) —
  "as if he had lost all his health" is read as *the same recall behavior*, since
...
Per stage, 16 rows (states named per repo convention — base name is the *toward* view,
...
~1.4 long / 0.9 tall; adult ~2.1 long / 1.1 tall. Geometry is *measured*, never declared
...
*backward* — the classic "backwards knee" that is actually a heel), and two folded
wing-arms.
...
- **Feather placement is an invariant** (Signet hair-coverage precedent): pink crest
  pixels on the head region, pink on forearms, pink fan on tail tip, in *every* frame of
  *every* row, all three stages. Gate it (§5.5 G-FEATHER).
...
tail; (5) each attack readable as bite vs. arm-rake vs. leaping foot-strike *from the
still strip alone*; (6) stages look like the same animal at three ages (family
...
- [ ] **A1 — Rig + stage table.** `scripts/mongoArt.ts` skeleton: `MongoPose`,
  ...
  three view painters producing a *static standing pose* in all three views × three
...
fraction (`hp = ceil(fraction * newMax)`) so leveling never *hurts*.
...
- [ ] **G5 — Ability def + UI.** `src/abilities/mongo.ts` per §6; `AbilityId` union +
  ...
  `drawMongoIcon` at the *current stage*; same treatment on the mobile button in
