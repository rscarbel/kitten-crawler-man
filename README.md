...
```
src/
  game.ts               ← entry point, game loop, input handling
  GameMap.ts            ← map generation, tile grid, pathfinding
  ...
  creatures/
    ...
    Mob.ts              ← base class for all enemies
...
In this project the loop lives in `game.ts`:
...
The grid is stored as a 2D array in `GameMap.structure[row][col]`.
...
The dungeon is generated fresh each time you start or restart. The algorithm in `GameMap.ts`:
...
export class MyEnemy extends Mob {
  ...
  updateAI(targets: Player[]) {
    ...
    if (!nearest) { this.clearAStarPath(); this.doWander(); return; }
...
3. **Spawn it** — import `MyEnemy` in `game.ts` and add it to the `spawnMob` function or the mob arrays in the constructor and `resetGame()`.

---

## How to Add a New Tile Type

1. Add the name to the `FLOOR_TYPES` array in `GameMap.ts`
